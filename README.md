...
I built the storage layer for FlatWall **Private Cloud**, a private AI/quant research platform, using a disaggregated **3FS**-based architecture on **AWS EKS** with **EFA**-enabled networking, NVMe-backed storage nodes, and **FoundationDB** metadata. The benchmark suite is designed to compare Local NVMe, Raw EFA, FUSE, USRBIO, checkpointing, metadata-heavy workloads, scaling, and KVCache-like read/GC pressure. 
