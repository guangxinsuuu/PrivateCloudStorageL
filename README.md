...
I built the storage layer for FlatWall **Private Cloud**, a private AI and quantitative research platform. The core idea is a disaggregated, **3FS**-based architecture on **AWS EKS**, featuring **EFA**-enabled networking, NVMe-backed storage nodes, and **FoundationDB** for metadata. The benchmark suite is designed to compare Local NVMe, raw EFA, FUSE, USRBIO, checkpointing, metadata-heavy workloads, scaling, and KVCache-like read/GC pressure. 
